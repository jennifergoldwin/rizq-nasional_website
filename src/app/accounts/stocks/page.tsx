...
const Page = () =>{
    ...
    const { register, handleSubmit, setError, formState: { errors } } = useForm<AddStockForm>();
    ...
    React.useEffect(() => {
      ...
      const fetchStock = async (token: string) => {
        ...
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_BASEURL}/all-stocks`, {
            method: 'GET',
            headers: {
    ...
    const onUpdatePrice = async (data: Stocks) => {
        try {
            const token = Cookies.get(cookiesAdmin.token) || "";
            if (token==="") return;
            const response = await fetch(`${process.env.NEXT_PUBLIC_BASEURL}/update-stock`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    const onSubmit = async (data: AddStockForm) => {
        try {
            const token = Cookies.get(cookiesAdmin.token) || "";
            if (token==="") return;
            const response = await fetch(`${process.env.NEXT_PUBLIC_BASEURL}/add-stock`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
                        ...
                        <form className="p-4 md:p-5">
                            <div className="grid gap-4 mb-4 grid-cols-2">
                                <div className="col-span-2">
                                    <label htmlFor="updatePrice" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Update Stock Price</label>
                                    <input onChange={(e:any)=>{
                                        let temp = updateStock;
                                        temp!.currPrice = parseInt(e.target.value);
                                    }} defaultValue={updateStock?.currPrice} type="number" name="updatePrice" id="updatePrice" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Stock Price" required/>

                                </div>
                            </div>
                            <button onClick={()=>onUpdatePrice(updateStock!)}  type="submit" className="text-white inline-flex items-center bg-[#5A64C3] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-[#5A64C3] dark:focus:ring-blue-800">
                                Update price
                            </button>
                        </form>
                    </div>
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="p-4 md:p-5">
                            <div className="grid gap-4 mb-4 grid-cols-2">
                                <div className="col-span-2">
                                    ...
                                    <input {...register('id', { required: 'Stock id is required' })} type="text" name="id" id="id" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Stock Id" required/>
                                ...
                                <div className="col-span-2 sm:col-span-1">
                                    ...
                                    <input {...register('stockName', { required: 'Stock name is required' })} type="text" name="stockName" id="stockName" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Stock Name" required/>
                                    {errors.stockName && <p className="text-red-600 text-sm">{errors.stockName.message}</p>}
                                </div>
                                <div className="col-span-2 sm:col-span-1">
                                    <label htmlFor="price" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Price</label>
                                    <input {...register('currPrice', { required: 'Stock price is required' })} type="number" name="currPrice" id="currPrice" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Price" required/>
                                    {errors.currPrice && <p className="text-red-600 text-sm">{errors.currPrice.message}</p>}
                                </div>
