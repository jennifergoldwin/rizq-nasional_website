...
const DoughnutChart: React.FC<DoughnutProps> = ({ totalDeposit = 0, totalProfit = 0 }) => {

    const chartRef = React.useRef<any>(null);
    const centerTextOptions = React.useRef<{ text: string }>({
        text: `RM${totalDeposit + totalProfit}`,
      });
    ...
    if (chartRef.current) {
      ...
      centerTextOptions.current.text = `RM${totalDeposit + totalProfit}`;
    ...
    return (
        <Doughnut ref={chartRef} options={{responsive: true, cutout: '70%', aspectRatio : 1.23}}
        plugins={[
            {
                id: 'centerText',
